...
d3 = RoundNew(u(10), decimalpoints);

% Parasitic elements
% par_length = RoundNew(u(14), decimalpoints);
% par_width = par_length;
% dPar1 = RoundNew(u(15), decimalpoints);
% dPar2 = RoundNew(u(16), decimalpoints);

% Substrate dimensions
substrate_length = RoundNew(1.1 * (arm_length + arm_width), decimalpoints);
substrate_width = RoundNew(1.1 * (arm_width + 5*U1B_width+7*(d1+d2+d3)), decimalpoints);
...
% Parasitic elements
% hfssRectangle(fid, 'Parasitic1', 'Z', [arm_length/2-arm_width,  -substrate_width/2+dPar1, total_thickness], par_length, par_width, 'mm');
% hfssSetColor(fid, 'Parasitic1', [128 , 0, 0]);
% hfssSetTransparency(fid, {'Parasitic1'}, 0);
% 
% hfssRectangle(fid, 'Parasitic2', 'Z', [-arm_length/2, - substrate_width/2+dPar2, total_thickness], par_length, par_width, 'mm');
% hfssSetColor(fid, 'Parasitic2', [128 , 0, 0]);
% hfssSetTransparency(fid, {'Parasitic2'}, 0);
...
% Array
hfssDuplicateAlongLine(fid, {'mmWaveAntenna'}, [0, -d1-2*U1B_width, 0], 2, 'mm');
hfssDuplicateAlongLine(fid, {'mmWaveAntenna'}, [0, d2+2*U1B_width, 0], 2, 'mm');
hfssDuplicateAlongLine(fid, {'mmWaveAntenna'}, [0, -d3-4*U1B_width, 0], 2, 'mm'); 
hfssDuplicateAlongLine(fid, {'mmWaveAntenna'}, [0, d3+4*U1B_width, 0], 2, 'mm');

hfssDuplicateAlongLine(fid, {'slot'}, [0, -d1-2*U1B_width, 0], 2, 'mm');
hfssDuplicateAlongLine(fid, {'slot'}, [0, d2+2*U1B_width, 0], 2, 'mm');
hfssDuplicateAlongLine(fid, {'slot'}, [0, -d3-4*U1B_width, 0], 2, 'mm');
hfssDuplicateAlongLine(fid, {'slot'}, [0, d3+4*U1B_width, 0], 2, 'mm');
...
% hfssAssignFiniteCondNew(fid, 'finiteConductivity8', 'Parasitic1', 'copper', 0);
% hfssAssignFiniteCondNew(fid, 'finiteConductivity9', 'Parasitic2', 'copper', 0);
